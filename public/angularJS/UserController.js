...
app.controller('UserController', function($scope, $http, API) {
    ...
    */
    $http.get(API + 'customer_group').then(function (response) {
        $scope.customer_groups = response.data;
    });
...
$('#readUser').on('show.bs.modal', function (event) {
    var modal = $(this);
    modal.find('.modal-title').text('Xem thông tin người dùng');
    ...
    modal.find('#customer_groupUser').attr('disabled', true);
    ...
    modal.find('#updateUser').click(function () {
        modal.find('.modal-title').text('Sửa thông tin người dùng');
        ...
        modal.find('#customer_groupUser').removeAttr('disabled', true);
